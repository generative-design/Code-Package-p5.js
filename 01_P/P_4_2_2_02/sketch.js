...
function setup() { 
  ...
  myCam = createCapture(VIDEO, function(){streamReady = true});
  myCam.hide();
...
function draw() { 
  if(streamReady) { 
    image(myCam, 0, 0, width, width*myCam.height/myCam.width); 
    ...
    if (interval == 0 && doSave == true) {
      var string = "" + counter;
      var pad = "00000";
      var ans = pad.substring(0, pad.length - string.length) + string;
      var saveFileName = str(startTime + "-" + ans);
